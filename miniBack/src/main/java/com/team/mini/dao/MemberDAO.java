...
public class MemberDAO {
    ...
    // 비밀번호 찾기 이메일로 회원가입 여부 확인
    public boolean regMemberCheck(String email) {
        boolean isNotReg = false;
        try {
            conn = Common.getConnection();
            stmt = conn.createStatement();
            String sql = "SELECT * FROM USERTB WHERE EMAIL = " + "'" + email + "'";
            rs = stmt.executeQuery(sql);
    ...
    // 임시비밀번호 발급을 눌렀을 때 DB의 원래 비밀번호를 tempPw로 바꿔주는 기능
    public void changeTempPw(String email, String tempPw) {
        ...
        try {
            ...
            if (rs.next()) {
                sql = "UPDATE USERTB SET PASSWORD=? WHERE EMAIL=?";
                pStmt = conn.prepareStatement(sql);
